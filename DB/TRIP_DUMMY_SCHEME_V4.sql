...
CREATE TABLE IF NOT EXISTS `trip_dummy`.`attractions` (
  ...
  INDEX `attractions_content_id_area_code` (`content_id` ASC, `area_code` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `trip_dummy`.`attraction_features` (
  ...
  INDEX `idx_attraction_features_attraction_id` (`attraction_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `trip_dummy`.`storybooks` (
  ...
  `content` LONGTEXT NULL DEFAULT NULL COMMENT '마크다운 포맷의 본문',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `trip_dummy`.`plan_details` (
  ...
  INDEX `idx_plan_details_attraction_id` (`attraction_id` ASC) VISIBLE,
  CONSTRAINT `fk_plan_details_attractions`
    FOREIGN KEY (`attraction_id`)
    REFERENCES `trip_dummy`.`attractions` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
